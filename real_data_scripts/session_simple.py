...
if __name__ == '__main__':
    ...
    trial_times = session.get_aligned_times(trial_numbers,
                                            time_before_in_s=2, # 2 sec before stim. change
                                            time_after_in_s=2, # to 2 sec after stim. change
                                            event='stimChange')
    # Spikes
    all_spikes = session.spike_time_stamps

    mu_results = np.ones((len(selected_unit_ind),len(trial_times),2)) * 2_000
    ...
    # plt.plot(np.zeros_like(blub),blub,ls="",marker='o')
    # plt.plot(np.ones_like(blub2),blub2,ls="",marker='o')
    plt.show()
    exit()
